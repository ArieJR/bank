...

{
    ...
    if(true) //connect buttons
    {
        ...
        buttonContent(Dbutton_2, "[D] Backspace");
        ...
        connect(oneButton_6, SIGNAL (released()), this, SLOT (gotoWelkomScherm()));
        connect(fourButton_6, SIGNAL (released()), this, SLOT (gotoWelkomScherm()));
        buttonContent(oneButton_6, "[1] yes");
        buttonContent(fourButton_6, "[4] no");
        labelContent(textTwo_6, "do you want a receipt?");
    }
...
void Login::readArduino()
{
    if (arduino->isOpen())
    {
        ...
        if(dataRead.size() <= 8)
        {
            ...
            //qDebug() << DataAsString;
            bool ok;
            if(DataAsString != "\r" && DataAsString != "\n")
            {
...
void Login::keyInput(int idx, int key) //idx is the current page idx, key is the input key
{
    ...
    else
    {
        ...
        if((idx == 3) && (key >= 0 && key <= 9) && (withdrawInput == true))
        {
            QString s = QString::number(key);
            inputBox_4->insert(s);
            return;
        ...
        if((idx==3) && (key == 13))
        {
            if(withdrawInput==true)
            {
                withdrawInput = false;
                labelContent(textFour_4,"pick an option, or choose other");
            }
            else
            {
                withdrawInput = true;

                labelContent(textFour_4,"fill in an amount");
            }
        }
        if (idx == 0 && key == 1)
        {
        ...
        if (idx == 3 && key == 14)
        {
            ...
            if(key == 1 || key == 4 || key == 7 || (key > 13 && key < 16))
            {
                ...
                if(idx == 3 && (key == 1 || key == 4 || key == 7|| key == 14|| key == 15)&&(withdrawInput==false))
                {
...
void Login::confirmWithdrawAmount()
{
    ...
    int amount = (inputBox_4->text()).toInt();
    ...
    else
    {
        ...
        {
            //wrongMult = true;
            labelContent(textTwo_4, "graag een veelvoud van €10,-");
            textTwo_4->show();
        }
        if(checkBalance(amount) != true)
        {
            labelContent(textThree_4, "onvoldoende saldo");
            textThree_4->show();
...
void Login::loginAttempt()
{
    ...
    {
        if(gebruikDatabase->checkPogingen()==1)
        {
            //errorMethod(2, textTwo_2);
            labelContent(textTwo_2, "Foute pincode: nog 2 pogingen.");
            textTwo_2->show();

        }
        if(gebruikDatabase->checkPogingen()==2)
        {
            //errorMethod(2, textTwo_2);
            labelContent(textTwo_2, "Foute pincode: nog 1 poging");
            textTwo_2->show();
        ...
        {
            returnCard();
            labelContent(textTwo, "Pas geblokkeerd");
            textTwo->show();
            endSession();
        }
    }
